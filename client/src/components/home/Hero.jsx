    ...
    if (searchMethod === 'image') {
      ...
      try {
        ...
        if (response.status === 401) {
          // If response status is 401 (Unauthorized), redirect to login page
          toast.error('Session Error. Please log in again.', {
            position: "top-center", // Middle of the screen
            autoClose: true,        // Prevent auto-close
            hideProgressBar: true,   // Hide the progress bar
            closeOnClick: false,     // Disable closing the toast by clicking anywhere
            closeButton: true,      // Hide the close (X) button
            style: {
              marginTop: '150px',    // Adjust to position the toast slightly down (middle and down)
              padding: '20px',       // Padding for the toast
              borderRadius: '8px',   // Rounded corners for a soft look
              boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)', // Subtle shadow to lift the toast
        ...
        <div className="max-w-3xl mx-auto text-center mb-10">
          ...
          <div className="bg-white p-6 rounded-xl shadow-xl">
            <div className="flex border-b border-gray-200 mb-6">
              <button
                className={`flex items-center px-4 py-3 font-medium border-b-2 transition-colors -mb-px text-gray-800 ${activeTab === 'text'
                    ? 'border-blue-600'
              ...
              <button
                className={`flex items-center px-4 py-3 font-medium border-b-2 transition-colors -mb-px text-gray-800 ${activeTab === 'image'
                    ? 'border-blue-600'
                    : 'border-transparent hover:border-gray-300'
                  }`}
              ...
              {activeTab === 'text' ? (
                ...
                <div className="animate-fade-in">
                  ...
                  <div className="flex space-x-4 mb-6">
                    <button
                      className={`flex items-center justify-center px-4 py-2 rounded-md flex-1 transition-colors ${uploadMethod === 'file'
                    ...
                    <button
                      className={`flex items-center justify-center px-4 py-2 rounded-md flex-1 transition-colors ${uploadMethod === 'url'
                          ? 'bg-blue-600 text-white'
                          : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                        }`}
